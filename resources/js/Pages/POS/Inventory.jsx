...
function StockBar({ qty, minQty }) {
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-500 tabular-nums whitespace-nowrap">{qty}/{minQty}</span>
        </div>
    );
}
...
function AddItemModal({ onClose, onAdd }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md bg-slate-800 border border-slate-700/60 rounded-2xl shadow-2xl overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-700/60 flex items-center justify-between">
                    <h2 className="text-white font-bold text-sm">Add New Item</h2>
                    ...
                    {[
                        ...
                        <div key={key}>
                            ...
                            <input
                                ...
                                className="w-full bg-slate-900/60 border border-slate-600/60 rounded-xl px-3 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 transition-colors"
                                placeholder={label}
                            />
                        </div>
                    ))}
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select
                                ...
                                className="w-full bg-slate-900/60 border border-slate-600/60 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-indigo-500 transition-colors appearance-none"
                            >
            ...
            <div className="flex flex-col h-full">
                ...
                <header className="px-6 py-4 bg-slate-800/80 border-b border-slate-700/60 flex items-center justify-between flex-shrink-0 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-slate-400 text-xs mt-0.5">{items.length} items · LKR {totalValue.toLocaleString()} total value</p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
                        {[
                            { label: 'Total Items', value: items.length, icon: Package, color: 'text-indigo-400', bg: 'bg-indigo-500/10' },
                            { label: 'Total Value', value: `LKR ${(totalValue / 1000).toFixed(0)}k`, icon: TrendingUp, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
                            { label: 'Low Stock', value: lowStockCount, icon: AlertTriangle, color: 'text-amber-400', bg: 'bg-amber-500/10' },
                            { label: 'Critical', value: criticalCount, icon: TrendingDown, color: 'text-red-400', bg: 'bg-red-500/10' },
                    ...
                    <div className="bg-slate-800 border border-slate-700/60 rounded-2xl p-4 mb-5">
                        <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            <select
                                ...
                                className="bg-slate-700/60 border border-slate-600/40 rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors appearance-none flex-shrink-0"
                            >
                                ...
                                <thead>
                                    <tr className="border-b border-slate-700/40 text-left">
                                        {['Item', 'Category', 'Stock Level', 'Price', 'Status', 'Supplier', '', ''].map((h, i) => (
                                            <th key={i} className="px-5 py-3 text-slate-500 text-xs font-semibold uppercase tracking-wider whitespace-nowrap">
                                                {h}
                                            </th>
                                        ))}
                                    </tr>
                                            ...
                                            <tr key={item.id} className="hover:bg-slate-700/20 transition-colors group">
                                                <td className="px-5 py-3.5">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <p className="text-white font-medium text-sm truncate max-w-[160px]">{item.name}</p>
                                                            <p className="text-slate-500 text-xs">{item.lastOrder}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-5 py-3.5">
                                                    <span className="text-slate-400 text-xs capitalize">{item.category}</span>
                                                </td>
                                                <td className="px-5 py-3.5 w-40">
                                                    <StockBar qty={item.qty} minQty={item.minQty} />
                                                    <p className="text-slate-500 text-xs mt-1">{item.qty} {item.unit}</p>
                                                </td>
                                                <td className="px-5 py-3.5">
                                                    <span className="text-white font-semibold tabular-nums">LKR {item.price.toLocaleString()}</span>
                                                ...
                                                <td className="px-5 py-3.5">
                                                    <span className="text-slate-400 text-xs">{item.supplier}</span>
                                                </td>
                                                <td className="px-5 py-3.5">
                                                    <button
