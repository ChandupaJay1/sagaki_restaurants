...
const TIER_CONFIG = {
    gold: { label: 'Gold', icon: Crown, color: 'text-amber-400', bg: 'bg-amber-500/15', border: 'border-amber-500/30' },
    silver: { label: 'Silver', icon: Star, color: 'text-slate-300', bg: 'bg-slate-400/15', border: 'border-slate-400/30' },
    bronze: { label: 'Bronze', icon: Heart, color: 'text-orange-400', bg: 'bg-orange-500/15', border: 'border-orange-500/30' },
};
...
function CustomerModal({ customer, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg bg-slate-800 border border-slate-700/60 rounded-2xl shadow-2xl overflow-hidden">
                {/* Header */}
                <div className="px-6 py-4 border-b border-slate-700/60 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-white font-bold text-base">{customer.name}</h2>
                ...
                {/* Content */}
                <div className="px-6 py-5 min-h-56">
                    {activeTab === 'details' && (
                        <div className="space-y-4">
                            ...
                            <div className="bg-slate-700/40 rounded-xl p-3">
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-slate-500 text-xs">Favorite Dish</span>
                                </div>
                                <p className="text-white font-medium text-sm">{customer.favorite}</p>
                            </div>
                            {customer.notes && (
                                <div className="bg-slate-700/40 rounded-xl p-3">
                                    <div className="flex items-center gap-2 mb-1">
                            ...
                            <div className="flex gap-2">
                                <a href={`tel:${customer.phone}`} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-sm font-medium rounded-xl transition-colors">
                                    <Phone size={14} /> Call
                                </a>
                                <a href={`mailto:${customer.email}`} className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-indigo-500/15 border border-indigo-500/30 text-indigo-400 text-sm font-medium rounded-xl transition-colors">
                                    <Mail size={14} /> Email
                                </a>
                            </div>
                        </div>
                    ...
                    {activeTab === 'history' && (
                        <div className="space-y-3">
                            {[
                                ...
                                <div key={i} className="flex items-center gap-4 bg-slate-700/30 rounded-xl p-3">
                                    <span className="text-slate-500 text-xs w-20">{order.date}</span>
                                    <span className="bg-indigo-500/15 text-indigo-400 text-xs font-semibold px-2 py-0.5 rounded-full">{order.table}</span>
                                    <span className="text-slate-400 text-xs">{order.items} items</span>
                                    <span className="text-white font-semibold text-sm ml-auto">LKR {order.total.toLocaleString()}</span>
                    ...
                    {activeTab === 'notes' && (
                        <div className="space-y-3">
                            <textarea
                                ...
                                rows={4}
                                className="w-full bg-slate-700/50 border border-slate-600/50 rounded-xl px-3 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none transition-colors"
                            />
                            <button
...
export default function CRM() {
    ...
    return (
        <POSLayout>
            ...
            <div className="flex flex-col h-full">
                ...
                <header className="px-6 py-4 bg-slate-800/80 border-b border-slate-700/60 flex items-center justify-between flex-shrink-0 backdrop-blur-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-bold text-white">Customer Relations</h1>
                            <p className="text-slate-400 text-xs mt-0.5">{stats.total} customers · LKR {stats.totalSpend.toLocaleString()} total spend</p>
                        </div>
                    </div>
                    <button
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
                        {[
                            { label: 'Total Customers', value: stats.total, icon: Users, color: 'text-indigo-400', bg: 'bg-indigo-500/10' },
                            { label: 'Gold Members', value: stats.gold, icon: Crown, color: 'text-amber-400', bg: 'bg-amber-500/10' },
                            { label: 'Total Revenue', value: `LKR ${(stats.totalSpend / 1000).toFixed(0)}k`, icon: TrendingUp, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
                            { label: 'Avg. Spend', value: `LKR ${stats.avgSpend}`, icon: Gift, color: 'text-violet-400', bg: 'bg-violet-500/10' },
                        ].map(({ label, value, icon: Icon, color, bg }) => (
                            <div key={label} className={`bg-slate-800 border border-slate-700/60 rounded-xl p-4 flex items-center gap-3`}>
                    ...
                    {/* Filters */}
                    <div className="bg-slate-800 border border-slate-700/60 rounded-2xl p-4 mb-5">
                        <div className="flex flex-col sm:flex-row gap-3">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-900/60 border border-slate-600/60 rounded-xl pl-9 pr-4 py-2.5 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition-colors"
                                />
                            </div>
                                    ...
                                    <button
                                        ...
                                        className={`
                                            ...
                                            ${tierFilter === t
                                                ...
                                                : 'bg-slate-700/60 text-slate-400 hover:text-white border border-slate-600/40'
                                            }
                                        `}
                                    >
                        ...
                        {filtered.map(customer => {
                            ...
                            return (
                                <button
                                    ...
                                    className="
                                        bg-slate-800 border border-slate-700/60 rounded-2xl p-5 text-left
                                        hover:border-slate-600/80 hover:bg-slate-750
                                ...
                                >
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mb-4">
                                        ...
                                        <div>
                                            <p className="text-slate-500 text-xs">Spend</p>
                                            <p className="text-indigo-400 font-bold text-sm">LKR {(customer.totalSpend / 1000).toFixed(1)}k</p>
                                        </div>
                                        <div>
                                            <p className="text-slate-500 text-xs">Last</p>
                                            <p className="text-white font-bold text-sm">{customer.lastVisit.slice(5)}</p>
                                        </div>
                                    </div>

                                    <div className="flex items-center justify-between pt-3 border-t border-slate-700/40">
                                        <span className="text-slate-400 text-xs truncate">🍽️ {customer.favorite}</span>
                                        <TierBadge tier={customer.tier} />
                                    </div>
                                </button>
                            );
            ...
            {toast && (
                <div className="fixed bottom-6 right-6 z-50 bg-slate-700 border border-slate-600/60 text-white text-sm font-medium px-4 py-3 rounded-xl shadow-2xl flex items-center gap-2">
